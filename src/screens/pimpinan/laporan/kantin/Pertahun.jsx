...
const Pertahun = () => {
  ...
  useEffect(() => {
    if (tahun) {
      setBukuKasTahun({tahun, kantin: true});
    }
  }, [tahun]);
  //   download file
  const REMOTE_IMAGE_PATH = `${BASE_URL}/export/pdf/tahun?tahun=${tahun}&kantin=kantin`;
  ...
  const downloadFile = () => {
    ...
    let options = {
      ...
      addAndroidDownloads: {
        ...
        notification: true,
        path: FileDir + `/bas-aplikasi/Buku Kas Kantin ${tahun}.pdf`,
        description: 'PDF',
      },
    };
    config(options)
