...
const Pertahun = () => {
  ...
  //   download file
  const REMOTE_IMAGE_PATH = `${BASE_URL}/export/pdf/bulan?bulan=${bulan}&tahun=${tahun}&kantin=kantin`;
  ...
  const downloadFile = () => {
    ...
    let options = {
      ...
      addAndroidDownloads: {
        ...
        notification: true,
        path: FileDir + `/bas-aplikasi/Buku Kas Kantin ${bulan}-${tahun}.pdf`,
        description: 'PDF',
      },
    };
    config(options)
  ...
  return (
    <View className="h-full">
      ...
      {tahun && bulan && dtBukuKas.length > 0 ? (
        <View>
