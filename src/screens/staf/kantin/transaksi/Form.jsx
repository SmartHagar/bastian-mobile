...
const Form = ({
  ...
  const cekKode = async () => {
    // kode baru
    let kode = '';
    const {data} = await setTransaksi();
    if (data.length > 0) {
      const filter = data.filter(row =>
        row.item.nama.toLowerCase().includes('kantin'),
      );

      console.log('filter', filter.length);

      if (filter.length > 0) {
        setPilihItem({value: filter[0].item.id});
        kode = `${moment(date).format('DD/MM/YYYY')}-${filter[0].item.kode}`;
        const no_urut = kodefikasi({
          old_prefix: data,
          new_prefix: kode,
        });
        setKode(kode);
