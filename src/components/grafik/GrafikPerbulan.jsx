...
const GrafikPerbulan = () => {
  ...
  function groupBy(items) {
    ...
    groupArrays.forEach(el => {
      const tgl = moment(el.tgl_transaksi).format('DD');
      el.data.forEach(row => {
      ...
      if (pemasukan.length < pengeluaran.length) {
        pemasukan.push({x: tgl, y: 0});
      }
      if (pengeluaran.length < pemasukan.length) {
        pengeluaran.push({x: tgl, y: 0});
      }
    });
  ...
  return (
    <View className="flex-1 w-screen">
      ...
      <View className="mt-2 flex-row space-x-2 justify-center">
        <View className="w-[50%]">
          <MonthSelect pilihBulan={setBulan} />
        </View>
        <View className="w-[40%]">
